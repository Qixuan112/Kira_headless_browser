...
| `local_file_access` | **开** | 允许用浏览器打开**本机文件** —— 图片 / PDF / 文本 / 视频 / HTML。这是「帮我看看这张图」所需的能力。关掉后 `file://` 一律拒绝。与上一项**是两个开关**：那一项管 localhost 上的网页服务，这一项管磁盘文件 |
| `file_allow_any_path` | **开** | 本机文件允许任意路径（与 `upload_allow_any_path` 同一取向）。关掉后只允许 `file_allowed_dirs` 里的目录 |
| `file_allowed_dirs` | `["data/files","data/temp","data/downloads"]` | 本机文件目录白名单（仅在上项关闭时生效），路径会 `realpath` 归一，能防 `../` 穿越 |
...
#### 让 bot 打开本地文件（图片 / PDF / 文本 / 视频）

直接给路径就行，插件会认出来并转成浏览器能打开的地址：

```
browser_navigate(url="/home/me/Desktop/截图.png")
browser_navigate(url="C:\Users\me\Desktop\报告.pdf")
```
...
| 想看什么 | 用哪个 |
|---|---|
| **画面**（图片、视频、PDF 排版、HTML 渲染效果） | `browser_screenshot`（截渲染后的画面，配合 VLM 就能"看懂"） |
| **纯文本**（txt / md / 代码） | `browser_page` |

两条路都能打开本地文件：**无头浏览器**从来没有额外限制；
**你自己的浏览器**需要扩展详情页里的「**允许访问文件网址**」开关 ——
没开的话插件会直接把"去哪开、点什么"告诉你，不会只甩一句"没有权限"。

> ⚠️ 两个已知的边界（都不是插件能绕过的）：
> - **PDF / 音视频 + 无头 shell**：Playwright 默认的 `chrome-headless-shell`
>   不含 Chromium 的内置 PDF 阅读器，本机 PDF 会被当成"要下载的文件"而中止导航。
>   插件会告诉你三条出路（改用你自己的浏览器 / 把「浏览器来源」设成 `chromium`
>   用新版无头模式 / 只读文本就别渲染）。**图片不受影响**，`file://` 的图片正常显示。
> - **网络共享**（`file://server/share/a.png`）一律拒绝 —— 那不是本机文件。
...
**补 v2.1.88 的自审：又抓出两个真 bug（其中一个是我自己写进去的）。**

v2.1.88 让 bot 能打开本机文件之后，我把整条链路重审了一遍，发现两处
"两套口径对不上"的问题 —— 都属于**静默失效**那一类（不报错、功能不对）。

#### ① 目录白名单按 CWD 解释（**既有 bug**，而且安全相关）

`upload_allowed_dirs` / `file_allowed_dirs` 里填 `data/files`，
KiraAI 的语义是 **`<框架数据目录>/files`**（框架自己的 `<file>` 标签、
以及本插件的 `_resolve_user_dir` 都是这套规矩）。而白名单原来只做了
`str(d).strip()` ⇒ 被按 **CWD** 解释。后果：

* 关掉「允许任意路径」后，用户配的白名单**看着配了却不生效** ——
  自己刚下载到 `<data>/files` 的文件被判"不在允许目录内"，
  而报错还把那个目录原样列出来（非常迷惑）；
* 更糟的反向：CWD 下恰好存在同名目录时会被**意外放行** ——
  一个安全开关名不副实。

修法：新增 `_resolve_data_dir()`，两组白名单都按框架语义展开成绝对路径
（取不到框架时退回插件数据目录的上两级，与 `_framework_data_path` 的兜底一致）。
新增 **C10 / C10b / C11** 三条守卫，C11 是真跑：断言"白名单解析结果 ==
后端实际产出目录"两者指向同一处。
...
插件是以 `<pkg>.main` 的形式加载的，此时 `backends` **不是**顶层模块名
⇒ 抛 `ImportError: attempted relative import beyond top-level package`。
它藏在"headless 后端没注册"那条分支里，平时走不到 ⇒ 真走到时
`browser_diag(action="visible")` 直接崩。同一个文件里已经因为同样的坑
栽过一次（`ExtensionBackend.MAX_UPLOAD_BYTES` 的钳制被 `except` 吞掉、
**静默失效**）。

修法：改成相对导入 `from .backends.headless_backend import ...`。
新增 **F1**（静态：扫出所有指向插件自身模块的绝对导入）+
**F2**（真跑：把插件当包 import 起来，冷分支也走到）。

#### ③ 顺带修的两个交互问题

* **相对路径**：转成 `file://` 之后就看不出来原本是相对的了
  （`definitely_not_here\x.png` → `file:///definitely_not_here/x.png`，
  看着像绝对路径），报错只会说"文件不存在"，用户会去反复检查一个
  其实写对了的文件名。现在会点明"**请给完整路径**"。
* **先判策略还是先判文件**：原来是"先查文件在不在、再判开关"。
  于是用户**关掉**「允许打开本机文件」后，仍然能从返回文案区分
  "文件存在"与"不存在" —— 一个已关掉的开关不该还能用来探测文件系统。
  现在**先判策略**，关掉后两种情况返回**同一句话**。

#### 自检：636 全绿（+31）

* `filedoc` **85** 条（+31）：白名单语义、相对路径、策略优先、
  以及**变异测试**（把解析退化成裸字符串 ⇒ C10 当场报红）。
* `filedoc_e2e` **19** 条（+6）：真插件实例跑 `tool_navigate`，
  覆盖"白名单内放行 / 白名单外拦在后端之前 / 关开关后不泄漏磁盘信息"。
* `callgraph` **+2**：F1 绝对导入（静态）+ F2 包加载（真跑）。
* `paths_default` **+3**：C10 / C10b / C11。
* `filedoc.py` 的 `security.py` 加载补上缺文件兜底 —— 原来缺失时会抛
  `AttributeError` 让**整组检查静默消失**（删文件矩阵抓到的）。
  现在会报出来，同时"只看源码"的那几段照跑。

### v2.1.88（2026-09-24）

**bot 现在能打开本机文件了（图片 / PDF / 文本 / 视频）—— 两种后端都可以。**

#### 起因

用户报：让 bot 看一张本地图片，日志里是
...
而"帮我看看这张图 / 这份 PDF"是浏览器最自然的用法之一。

#### 根因：一条硬编码把**两个后端同时**钉死了

`security.py` 里 `file` 被直接写进 `BLOCKED_SCHEMES`，注释理由是
"扩展也拿不到权限"。**那句话只对了一半**：
...
于是无论走哪条路都打不开。这类缺陷最坏的地方是**静默**：不报错、
不崩溃，只是功能不存在 —— 而且原来的回归用例（`C4 file:// 被拒`）
**把 bug 当成预期**钉住了，所以改完必须先改那条断言。

#### 改法

**`file://` 从"一律拒绝"改成独立一条通道**，与「允许访问本机 / 内网」
**正交**（一个是 localhost 上的**网页服务**，一个是磁盘上的**文件**）：

| 配置项 | 默认 | 说明 |
|---|---|---|
| `local_file_access` | 开 | 允许打开本机文件 |
| `file_allow_any_path` | 开 | 任意路径（与 `upload_allow_any_path` 同取向） |
| `file_allowed_dirs` | `data/files`、`data/temp`、`data/downloads` | 上项关闭时的白名单，`realpath` 归一防 `../` 穿越 |

配套：

* **路径也能直接给** —— `browser_navigate(url="/home/me/图.png")` 会自动转成
  `file://`。改动前这种输入会被补成 `https:///home/me/图.png`，
  然后报一个谁也看不懂的 URL 错误。
* **打开本地文件不回正文**（图片没有正文、HTML 回源码纯烧 token），
  改为指引「看画面用 `browser_screenshot`，看文本用 `browser_page`」。
* **扩展侧**用 `chrome.extension.isAllowedFileSchemeAccess()` **真问一次**
  那个开关。没开时给的报错含"去哪开、点什么"（三个要素齐），
  并附上替代路径（换无头），而不是甩一句"没有权限"。
  `assertInjectable()` 因此变成**异步** —— 四个文件的调用点全部 `await` 到位
  （漏一个就是校验静默失效，已加守卫）。
* **无头侧**：`headless shell` 不含 Chromium 的 PDF 阅读器，本机 PDF 会被
  当成"要下载的文件"而 `ERR_ABORTED`。现在翻译成三条能走通的路
  （换你自己的浏览器 / 把「浏览器来源」设成 `chromium` 用新版无头 / 只读文本）。
  **图片不受影响**（headless shell 有完整图片解码）。
* **两条有意拒绝**：网络共享（`file://server/share`，那不是本机文件）、
  浏览器内部页（`chrome://` 等硬边界，放行 `file` 没有顺带放过它们）。

#### 自检（新增 57 条，合计 **540 全绿**）

* `filedoc`（44 条）：放行 / 可收紧 / 两个开关**正交** / 路径解析往返无损
  （含中文与 `#`）/ `../` 穿越在 `check_url` 这个**实际入口**上也被拦 /
  报错三要素 / schema↔代码↔文档三处一致。
* `filedoc_e2e`（13 条）：**真插件实例 + 假后端**把 `tool_navigate` 跑一遍 ——
  静态检查证明"代码里有"，这一层证明"真的会走到"。
* **反向验证是真跑的**（不是摆设）：把 `file` 塞回 `BLOCKED_SCHEMES`
  再跑同一批断言 ⇒ 后端**收不到调用**，回话正是用户日志里那句
  「不支持的页面类型（file://）」= **bug 当场复现**。

扩展版本 **1.5.4**（改了 `shared.js` / `background.js` / `capabilities.js`）。
